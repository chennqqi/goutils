...
func unescape(_s string, mode encoding) (string, error) {
	...
	for i := 0; i < len(s); {
		...
		case '%':
			...
			if mode == encodeHost && unhex(s[i+1]) < 8 && _s[i:i+3] != "%25" {
				return "", EscapeError(_s[i : i+3])
			}
			if mode == encodeZone {
				...
				if _s[i:i+3] != "%25" && v != ' ' && shouldEscape(v, encodeHost) {
